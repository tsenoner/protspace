...
class InterProFeatureRetriever:
    ...
    def __init__(self, headers: List[str] = None, features: List[str] = None, sequences: Dict[str, str] = None):
        """
        ...
        if invalid_features:
            logger.warning(f"Invalid InterPro features: {invalid_features}. Supported: {INTERPRO_FEATURES}")
    ...
    def fetch_features(self) -> List[NamedTuple]:
        ...
        if not self.sequences:
            logger.warning("No sequences provided for InterPro feature retrieval. MD5 calculation requires sequences.")
            return []
        ...
        if missing_sequences:
            logger.warning(f"Missing sequences for {len(missing_sequences)} proteins: {missing_sequences[:5]}...")
    ...
    def _get_matches_in_batches(self, md5s: List[str]) -> List[Dict]:
        ...
        logger.info(f"Submitting {len(md5s)} sequences to InterPro API in {len(chunks)} batch(es)...")

        with tqdm(total=len(md5s), desc="Fetching InterPro features", unit="seq") as pbar:
            for i, chunk in enumerate(chunks, 1):
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.debug(f"Batch {i}/{len(chunks)} successful. Received {len(batch_results)} results.")
                    else:
                        logger.error(f"Error processing batch {i}: {response.status_code} - {response.text}")
    ...
    def _parse_interpro_results(self, api_results: List[Dict], md5_to_identifier: Dict[str, str]) -> List[NamedTuple]:
        """
            ...
            for match in result.get("matches", []):
                ...
                if source_db in self.features:
                    ...
                    if signature_accession:
                        protein_features[protein_id][source_db].append(signature_accession)
        ...
        for identifier, features_dict in protein_features.items():
            ...
            result.append(ProteinFeatures(identifier=identifier, features=processed_features))
