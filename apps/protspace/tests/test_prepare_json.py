...
def test_files(
    ...
    invalid_sim = pd.DataFrame(
        similarity_matrices["euclidean"][:50], index=headers[:50], columns=headers
    )
...
@pytest.fixture
def processor():
    """Create a LocalDataProcessor instance."""
    return LocalDataProcessor({"random_state": RANDOM_SEED})


def test_load_embeddings_ordering(
    processor, test_files, headers, structured_embeddings
):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        metadata, data, loaded_headers = processor.load_data(
            ...
            str(test_files["metadata"]),  # Convert Path to string
            output_path,
            delimiter=",",
            non_binary=True,
            keep_tmp=False,
...
def test_partial_metadata(processor, test_files):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        metadata, data, headers = processor.load_data(
            test_files["embeddings"],
            str(test_files["partial_metadata"]),
            output_path,
            delimiter=",",
            non_binary=True,
            keep_tmp=False,
...
def test_different_metrics(processor, test_files, method, metric):
    ...
    processor = LocalDataProcessor({"random_state": RANDOM_SEED, "metric": metric})
...
def test_cluster_preservation(processor, test_files, method):
    ...
    metadata, data, headers = processor.load_data(
        test_files["embeddings"], test_files["metadata"]
    )
...
def test_single_cluster(processor, test_files, method):
    ...
    metadata, data, headers = processor.load_data(
        test_files["single_cluster"], test_files["metadata"]
    )
...
def test_sparse_metadata_handling(processor, test_files, sparse_metadata):
    ...
    metadata, data, headers = processor.load_data(
        test_files["embeddings"], test_files["sparse_metadata"]
    )
    ...
    pd.testing.assert_series_equal(
        metadata.loc[non_nan_idx, "value"], sparse_metadata.loc[non_nan_idx, "value"]
    )


@pytest.mark.parametrize(
    "input_type,meta_type",
    [
        ("embeddings", "metadata"),
...
def test_output_consistency(processor, test_files, headers, input_type, meta_type):
    ...
    metadata, data, loaded_headers = processor.load_data(
        test_files[input_type], test_files[meta_type]
    )
...
def test_error_handling(processor, test_files, error_file):
    ...
    metadata, data, headers = processor.load_data(test_files["embeddings"], error_path)
