...
Each call to `loadExampleDataset` bumps a request sequence number; a request bails out (returns `false`, touches nothing) as soon as it notices a later one has started, both after the fetch and after reading the response body. This drops a superseded example fetch — two menu/url requests in a row, or a user import/OPFS restore starting while one is in flight — so a slow, now-stale fetch can never resolve and overwrite what's on screen after a newer one already has.
...
- [Mocked `fetch` in unit tests doesn't prove the real deep-link flow works] → the Playwright spec covers the deep link, Back/Forward (including a dataset switch and a view-param change landing on the same step) and user import against the real bundles; `example-datasets.test.ts` also uses `import.meta.glob` (not `node:fs`, which `tsconfig.app.json` deliberately carries no types for) to confirm every catalog `url` resolves to a bundle that actually ships under `apps/web/public/`, and `example-datasets-docs.test.ts` does the same to pin the catalog against the ids documented in `control-bar.md`.
