...
- [ ] 1.1 `style-getters.test.ts`: hidden beats selected (selected id whose only value is
      hidden → opacity exactly 0)
- [ ] 1.2 `style-getters.test.ts`: selection fading × hidden composition (non-empty
...
- [ ] 1.3 `style-getters.test.ts`: vacuous-truth edge — point with zero annotation values
      (Int32Array sentinel `-1`) is hidden even with an empty hidden set
- [ ] 1.4 `style-getters.test.ts`: all-hidden escape hatch — opacity rescued to base tier;
      `getColors` returns `[]` (colors NOT rescued)
- [ ] 1.5 `style-getters.test.ts`: highlight-only state — highlighted point gets selected
      opacity, others keep base (no fading)
- [ ] 1.6 `data-processor.test.ts`: `visibleProteinIds` (5th param) × isolation-history
      intersection — both applied, global `originalIndex` preserved
- [ ] 1.7 `scatter-plot.filter-render.test.ts` (unattached-element pattern): apply query
...
- [ ] 1.8 `scatter-plot.filter-render.test.ts`: dataset swap clears
      `filteredProteinIds`/`filtersActive` before `_processData` consumes them (new data
      renders un-culled)

## 2. Pure module

- [ ] 2.1 Add `packages/core/src/components/scatter-plot/visibility-model.ts`:
...
- [ ] 2.2 Add `visibility-model.test.ts` covering every row of the design D5 semantics
      table (11 rules), including exact-0 assertions and `tierOf` never collapsing
      selected into base
- [ ] 2.3 Verify knip passes (`pnpm quality`): every export consumed

## 3. Delegate style getters

- [ ] 3.1 `createStyleGetters` gains an optional `model?: VisibilityModel` param —
      ...
      shape unchanged
- [ ] 3.2 Entire existing `style-getters.test.ts` suite green unchanged — especially
      depth-stability (`:201-245`) and `__NA__` normalization tests

## 4. Wire the component

- [ ] 4.1 Add memoized `_getVisibilityModel()` to `scatter-plot.ts`: keys =
      ...
      keys
- [ ] 4.2 Migrate the five predicate sites to `model.isInteractive(d)`: quadtree build
      (`:888`), lasso (`:1259`), brush (`:1276`), hover (`:1997`), click (`:2057`);
      keep the separate `isPointRendered` composition at hover/click (`:2003`, `:2062`)
- [ ] 4.3 Keep `_getOpacity` as a facade delegating to the model (external consumers:
      `webgl-render-perf.ts:555-566`, `app/tests/brush-selection.spec.ts:323-325`)
- [ ] 4.4 Confirm renderer invalidation call sites untouched (hidden-only →
      style-cache-only; annotation/Other → + position cache); no new invalidation added
- [ ] 4.5 All Section-1 pinned tests and existing suites green

## 5. Verification

- [ ] 5.1 `pnpm precommit` (format, lint, typecheck, vitest) green at repo root
...
- [ ] 5.4 Grep confirms no remaining ad-hoc visibility derivation:
