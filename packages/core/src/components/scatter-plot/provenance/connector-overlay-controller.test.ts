...
describe('ConnectorOverlayController', () => {
  ...
  it('scales the connector line stroke-width with the drawn radius', () => {
    controller.set({
      pairs: [{ sourceProteinId: 'source', targetProteinId: 'target', confidence: 0.8 }],
      totalCandidates: 1,
    });
    ...
    expect(Number(biggerLine?.getAttribute('stroke-width'))).toBeCloseTo(3, 10);
  });

  it('keeps a 4 px halo around dots drawn smaller than 2 px', () => {
    const tinyController = new ConnectorOverlayController({
      getOverlayGroup: () => overlay,
      getPlotData: () => plotData,
      getScales: () => scales,
      getPointRadiusPx: () => 1,
    ...
    tinyController.updateZoomScale(4);
    const radii = [...svg.querySelectorAll('circle.eat-provenance-endpoint')].map((c) =>
      Number(c.getAttribute('r')),
    );
    expect(radii).toEqual([1, 1]);
  });
